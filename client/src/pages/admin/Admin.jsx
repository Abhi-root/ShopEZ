...
const Admin = () => {
  ...
  const fetchCountData = async () => {
    try {
      const users = await axios.get('https://shopez-2-6e3z.onrender.com/api/users/fetch-users');
      setUserCount(users.data.length - 1);
      
      const products = await axios.get('https://shopez-2-6e3z.onrender.com/api/products/fetch-products');
      setProductCount(products.data.length);
      
      const orders = await axios.get('https://shopez-2-6e3z.onrender.com/api/orders/fetch-orders');
...
const updateBanner = async () => {
  try {
    ...
    await axios.post('https://shopez-2-6e3z.onrender.com/api/banners', { bannerUrl: banner });
