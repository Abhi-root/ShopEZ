...
const Cart = () => {
  ...
  const fetchCart = async () => {
    try {
      ...
      const response = await axios.get('https://shopez-2-6e3z.onrender.com/api/cart/fetch-cart', config);
  ...
  const removeItem = async (itemId) => {
    try {
      const config = { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } };
      await axios.delete(`https://shopez-2-6e3z.onrender.com/api/cart/remove-item/${itemId}`, config);
      fetchCart();
  ...
  const placeOrder = async () => {
    ...
    try {
      await axios.post('https://shopez-2-6e3z.onrender.com/api/orders/place-cart-order', {
        ...formData,
